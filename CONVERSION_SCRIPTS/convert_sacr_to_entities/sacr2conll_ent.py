...
def read_file(fpath, index, docname=None, part_is_index=True, include_speaker=False):
    ...
    for item, params in parser.parse():
        ...
        if item == "text_id":
            ...
            # print(params)
            chain = params[0]
            chain = "%s:%s:%s" % (params[0], params[1], re.sub(" ", "_", params[2]["EN"]))
            # print(chain)
            l = len(tokens)
            if l not in starts:
                starts[l] = []
    ...
    for i, (token, speaker) in enumerate(tokens):
        ...
        corefcol = "|".join(
            ...
            # + ["(%d" % x for x in (starts[i]
            ["(%s" % x for x in (starts[i] if i in starts else [])]
            + ["%s)" % x for x in (ends[i] if i in ends else [])]
        )
        corefcol = re.sub(r"\((.+)\|\1\)", r"(\1)", corefcol)
        if not corefcol:
            corefcol = "-"
        if include_speaker:
