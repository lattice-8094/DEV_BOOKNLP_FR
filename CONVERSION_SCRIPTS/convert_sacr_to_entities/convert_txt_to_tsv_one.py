...
class NestedNERGenerator:
    ...
    def findNested(self):
        ...
        for i in self.intervals:
            ...
            # print("appendingg", " ".join([i.split()[1], beginIdx, i.split()[0]]))
...
if __name__ == "__main__":
    ...
    for text, label in dataset:
        ...
        for l in layers:
            labelList = ['O' for i in range(1+len(textSplit))]
            for elem in l:
                label, textBeginIdx, textEndIdx = elem.split()[0], int(elem.split()[1]), int(elem.split()[2]) - 1 
                # print(label, textBeginIdx, textEndIdx, labelList)
