...
def get_max(pathToTSV):
	maxCols=0
	for filename in glob.glob(os.path.join(pathToTSV, '*.sacr.conll.data')):
		# print(filename)
...
def make_split(filename, outputFile, pathToTSV, maxCols):
	...
	with open(filename) as file:
		...
		for line in file:
			name=line.rstrip()
			path=os.path.join(pathToTSV, "%s.sacr.conll.data"%  name)
...
if __name__ == "__main__":
	...
	for split in "train", "dev", "test":
		outDir="%s/%s" % (outputDir, split)
		os.makedirs(outDir)
		inputPath=os.path.join(inputDir, "%s.ids" % split)
		outputPath=os.path.join(outDir, "%s.data" % split)
