...
import torch
import numpy as np
import multilingualbooknlp.common.crf as crf
import multilingualbooknlp.common.sequence_eval as sequence_eval
...
class Tagger(nn.Module):
	...
	def __init__(self, freeze_bert=False, base_model=None, tagset=None, supersense_tagset=None, tagset_flat=None, hidden_dim=100, flat_hidden_dim=200, device=None):
		...
		if modelName == "PlanTL-GOB-ES/roberta-base-bne":
			self.tokenizer = RobertaTokenizer.from_pretrained('PlanTL-GOB-ES/roberta-base-bne')
			self.bert = RobertaModel.from_pretrained('PlanTL-GOB-ES/roberta-base-bne')

		elif modelName=="google/mt5-base":
			self.tokenizer = T5Tokenizer.from_pretrained(modelName)
			self.bert = MT5EncoderModel.from_pretrained(modelName)
	...
	def forward(self, input_ids, matrix1, matrix2, attention_mask=None, transforms=None, labels=None, lens=None):
		...
		if self.modelName=="google/mt5-base":
	...
	def predict(self, input_ids, attention_mask=None, transforms=None, lens=None):
		...
		# if self.modelName=="google/mt5-base":
		# 	output = self.bert(input_ids, attention_mask=attention_mask, output_hidden_states=True, return_dict=False)
		# 	# out=outputs.last_hidden_state
		# else:
		# 	output = self.bert(input_ids, token_type_ids=None, attention_mask=attention_mask, output_hidden_states=True, return_dict=False)
