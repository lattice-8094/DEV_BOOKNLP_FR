...
from multilingualbooknlp.common.pipelines import Token, Entity
...
class BERTCorefTagger(nn.Module):
	...
	def __init__(self, gender_cats, freeze_bert=False, base_model=None, pronominalCorefOnly=True):
		...
		if modelName=="camembert-base":
			self.tokenizer = CamembertTokenizer.from_pretrained("camembert-base")
			self.bert = CamembertModel.from_pretrained("camembert-base")

		elif modelName == "DeepPavlov/rubert-base-cased":
		...
		# self.tokenizer = CamembertTokenizer.from_pretrained("camembert/camembert-base", do_lower_case=False, do_basic_tokenize=False)
		# self.bert = CamembertModel.from_pretrained("camembert/camembert-base")
		
		# self.tokenizer = AutoTokenizer.from_pretrained("flaubert/flaubert_base_cased", do_lower_case=False, do_basic_tokenize=False)
		...
		# e.g. self.gender_expressions["he/him/his"]={"he":1, "him":1, "his":1}
		...
		# e.g. self.conflicting_genders["he/him/his"]={"she":1, "her":1, "they":1, "them":1, etc.}			
	...
	def get_mention_reps(self, input_ids=None, attention_mask=None, starts=None, ends=None, index=None, widths=None, quotes=None, matrix=None, transforms=None, doTrain=True):
		...
		# flaubert
		# sequence_outputs, all_outs  = self.bert(input_ids, token_type_ids=None, attention_mask=attention_mask, output_hidden_states=True, return_dict=False)
	...
	def forward(self, matrix, index, existing=None, truth=None, token_positions=None, starts=None, ends=None, widths=None, input_ids=None, attention_mask=None, transforms=None, entities=None, ref_genders={}):
		...
		# during testing, doTrain is false
		...
		for b in range(len(matrix)):
			...
			# print("matrix", matrix[b].shape, input_ids[b].shape, transforms[b].shape)
	...
	def print_conllu(self, name, sents, all_ents, assignments, out, token_maps):
		...
		for s_idx, sent in enumerate(sents):
			...
			for w_idx, word in enumerate(sent):
				...
				out.write("%s\t%s\t_\t_\t_\t_\t_\t_\t_\t%s\n" % (tok_id+1, word, '_' if len(label) == 0 else '|'.join(label)))
				tok_id+=1
				cur_tok+=1
	...
	def get_data(self, doc, ents, max_ents, max_words, batchsize=32):
		...
		with open(filename, encoding="utf-8") as file:
			...
			for line in file:
				if line.startswith("#begin document"):
					...
					if cur_tokens >= max_allowable_tokens and open_count != 0 and cur_tokens > 500:
						print("CLOBBERING MENTION", open_ents)
						keys=open_ents.keys()
						for c in keys:
							open_ents[c]=[]
						open_count=0

						print("CUR TOKS", cur_tokens)
						# sys.exit(1)
	...
	def read_conllu(self, filename, quotes={}):
		...
		# for line in file:
			if lidx == len(data)-1 or line.startswith("# newdoc id = "):
				...
				if line.startswith("# newdoc id = "):
					...
					docid=None
					matcher=re.match("# newdoc id = (.*)$", line.rstrip())
					if matcher != None:
						docid=matcher.group(1)
						partID="None"#matcher.group(2)
			...
			# elif line.startswith("#end document"):
				...
				# print(orig_token_id, global_id, coref)
				if docid in quotes and sid in quotes[docid]:
	...
	def load_conllu(self, file_path):
		...
		sent2sent_in_doc={}
		sent_in_doc=0
		for idx, tree in enumerate(doc.trees):
			if tree.newdoc:
				curDoc=tree.newdoc
				sent_in_doc=0

			sents2doc[idx]=curDoc
			if curDoc not in doc2id:
				doc2id[curDoc]=len(doc2id)

			if curDoc not in doc2sents:
				doc2sents[curDoc]=[]
			doc2sents[curDoc].append(idx)

			for node in tree.descendants_and_empty:
				node_map[node]=idx
				if idx not in sents:
					sents[idx]=[]
				sents[idx].append(node.form)

			sent2sent_in_doc[idx]=sent_in_doc

			sent_in_doc+=1

		ents=[]
		for i in range(len(doc2sents)):
			ents.append([])

		for doc_n in doc2sents:
			docid=doc2id[doc_n]
			for i in range(len(doc2sents[doc_n])):
				ents[docid].append([])
		...
		for cluster in doc.coref_entities:
			...
			for mention in cluster.mentions:
				first, last=mention.words[0].ord, mention.words[-1].ord
				global_sentid=node_map[mention.words[0]]
				doc_n=sents2doc[global_sentid]
				sentid=sent2sent_in_doc[global_sentid]
				docid=doc2id[doc_n]

				ents[docid][sentid].append((first-1,last-1))
		...
		for doc_n in doc2sents:
			docid=doc2id[doc_n]
			doc_sents=[]
			for sentid in doc2sents[doc_n]:
				doc_sents.append(sents[sentid])

			all_sents.append(doc_sents)


		for docid in range(len(all_sents)):
			for sentid in range(len(all_sents[docid])):
				sent=all_sents[docid][sentid]
				entities=ents[docid][sentid]
				# print("SENT", sent)
				# print("ENTS", entities)
				# print()

		return all_sents, ents

	def read_conllu(self, filename, quotes):

		# do nothing with quotes yet

		all_doc_sents, all_ents=self.load_conllu(filename)
		...
		for docid in range(len(all_doc_sents)):
			sentence_breaks[str(docid)]={}
			cur=0
			for sentid in range(len(all_doc_sents[docid])):
				cur+=len(all_doc_sents[docid][sentid])
				sentence_breaks[str(docid)][cur]=1
				all_doc_sents[docid][sentid].insert(0, "[CLS]")
				all_doc_sents[docid][sentid].append("[SEP]")
			



		# get the global token ID (with respect to the document) for each word in each document
		token_mapper={}
		for docid in range(len(all_doc_sents)):
			all_doc_names.append((str(docid), "0"))
			all_docids.append(str(docid))

			gid=0
			for sentid in range(len(all_doc_sents[docid])):
				sent=all_doc_sents[docid][sentid]
				for wid, word in enumerate(sent):
					token_mapper[(docid,sentid,wid)]=gid
					gid+=1

		# print(token_mapper)

		# update the entity (start,end) positions, and +1 to account for [CLS]
		for docid in range(len(all_doc_sents)):
			for sentid in range(len(all_doc_sents[docid])):
				entities=all_ents[docid][sentid]
				new_entities=[]
				for start, end in entities:
					# gstart=token_mapper[(docid,sentid,start)]+1
					# gend=token_mapper[(docid,sentid,end)]+1
					# print(' '.join(all_doc_sents[docid][]))
					new_entities.append((start+1, end+1))

				all_ents[docid][sentid]=new_entities
		...
		for docid in range(len(all_doc_sents)):
			...
			for sentid in range(len(all_doc_sents[docid])):
				...
				sent_entity_objects=[]
				for start, end in entities:
					e=Entity(start, end, in_quote=False, quote_eid=None, quote_id=None, entity_id=0, text=' '.join(sent[start:end+1]))
					
					gstart=token_mapper[(docid,sentid,start)]+1
					gend=token_mapper[(docid,sentid,end)]+1
					e.global_start=gstart
					e.global_end=gend

					sent_entity_objects.append(e)

				all_doc_ents[docid].append(sent_entity_objects)

				

			doc_antecedent_labels, big_ents, max_words, max_ents, quote_antecedent_labels=self.get_ant_labels(all_doc_sents[docid], all_doc_ents[docid], None)
			processed_ents.append(big_ents)
			all_max_words.append(max_words)
			all_max_ents.append(max_ents)
			all_antecedent_labels.append(doc_antecedent_labels)

			print("w:", max_words, "e:", max_ents)
		
		all_quotes=None
		all_named_ents=None

		print("SENT", sentence_breaks)

		return all_doc_sents, processed_ents, all_named_ents, all_antecedent_labels, all_max_words, all_max_ents, all_doc_names, sentence_breaks, all_quotes, all_docids

if __name__ == "__main__":
	tagger=BERTCorefTagger()
	tagger.read_conllu(sys.argv[1])
