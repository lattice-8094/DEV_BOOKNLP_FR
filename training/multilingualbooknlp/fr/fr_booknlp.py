...
import spacy
import copy
from multilingualbooknlp.common.pipelines import SpacyPipeline
from multilingualbooknlp.fr.entity_tagger import LitBankEntityTagger
from multilingualbooknlp.fr.gender_inference_model_1 import GenderEM
from multilingualbooknlp.fr.name_coref import NameCoref
from multilingualbooknlp.fr.litbank_coref import LitBankCoref
from multilingualbooknlp.fr.litbank_quote import QuoteTagger
from multilingualbooknlp.fr.bert_qa import QuotationAttribution
...
class FrBookNLP:
	...
	def __init__(self, model_params):
		...
		with torch.no_grad():
			...
			spacy_model="fr_dep_news_trf"#"fr_core_news_sm"
			...
			valid_keys=set("entity,coref".split(","))
			
			pipes=model_params["pipeline"].split(",")

			self.gender_cats= [ ["male"], ["female"]] 
			...
			if model_params["model"] == "big":
				entityName="fr_catprop_camembert-base.model"
				corefName="fr.coref.model"
			...
			tagsetPath="/data/booknlp/JEAN_DEV_BOOKNLP_FR/training/TRAINING_DATA/ENTITIES/SACR_V2/fr.entities.tagset"
			#tagsetPath = pkg_resources.resource_filename(__name__, tagsetPath)
			print('tagsetPath :     ', tagsetPath)
	...
	def get_syntax(self, tokens, entities, assignments, genders):
		...
		for idx, (start_token, end_token, cat, phrase) in enumerate(entities):
			...
			if tok is not None:                
				...
				# mod
				if tok.deprel == "nsubj" and (head.pos == 'NOUN' or head.pos == 'ADJ'):
					mods[coref].append({"w": head.text, "i": head.token_id})
				
					for sibling in toks_by_children[head.token_id]:
						sibling_id = sibling.token_id
						sibling_tok = tokens[sibling_id]
						if (sibling_tok.deprel == "conj") and (sibling_tok.pos == "NOUN" or sibling_tok.pos == "ADJ"):
					...
					# agent oblique d'une passive
				elif tok.deprel == "obl:agent":
					for sibling in toks_by_children[head.token_id]:
						sibling_id = sibling.token_id
						sibling_tok = tokens[sibling_id]
						if sibling_tok.deprel == "aux:pass":
							agents[coref].append({"w": sibling_tok.text, "i": sibling_tok.token_id})
								
					#patient
				elif (tok.deprel == "obj" or tok.deprel == "nsubj:pass") and head.pos == "VERB":
					patients[coref].append({"w": head.text, "i": head.token_id})

					
				"""
				#poss ## a vérifier, compliqué de trouver un équivalent, pas de tok.morph ici à revoir quand l'on mettra stanza ?
				elif tok.deprel == "det" and 'Poss=Yes' in tok.morph:
		...
		with torch.no_grad():
			...
			with open(filename) as file:
				...
				if self.doEntities:
					...
					# only identify first-person narrator if quotation tagging is implemented (otherwise
					# any 1p pronoun will be considered the narrator)
		  ...
		  <meta charset="UTF-8">
						...
						valid_cats=set(["FAC", "GPE", "LOC", "PER", "ORG", "VEH"])
						major_places={}
						...
						for idx, (start, end, cat, text) in enumerate(entities):
							...
							ner_prop=cat.split("_")[0]
							ner_type=cat.split("_")[1]
							if ner_prop != "PRON" and ner_type in valid_cats:
