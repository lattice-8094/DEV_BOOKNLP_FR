...
class NameCoref:
	...
	def __init__(self, aliasFile):
		#self.honorifics={"monsieur":1, "mademoiselle":1, "madame":1, "mr":1, "mr.":1, "mrs":1, "mrs.":1, "miss":1, "uncle":1, "aunt":1, "lady":1, "lord":1, "master":1, "mistress":1}
		self.honorifics={"monsieur":1, "mademoiselle":1, "madame":1, "m":1, "m.":1, "mme":1, "mme.":1, "mlle.":1, "docteur":1, "dr":1, "dr.":1, "maître":1, "maîtresse":1, "oncle":1, "tante":1, "dame":1, 
				    "roi":1, "reine":1, "prince":1, "princesse":1, "duc":1, "duchesse":1, "marquis":1, "marquise":1, "comte":1, "comtesse":1, "vicomte":1,"vicomtesse":1, "baron":1, "baronne":1}
		self.aliases={}
	...
	def name_cluster(self, entities, is_named, existing_refs):
		...
		# each name needs to have been seen at least twice
		min_uniq_count=2

		uniq_min={}
		for name in uniq:
			if uniq[name] >= min_uniq_count:
				uniq_min[name]=uniq[name]
	...
	def read_file(self, spanFile):
		...
		with open(spanFile) as file:
			...
			for line in file:
				...
				entities.append((start, end, "%s_%s" % (prop, cat), text))
				is_named.append(1)
	...
	def process(self, spanFile, tokenFile):
		toks=self.read_tokens(tokenFile)
		entities, is_named=self.read_file(spanFile)
		refs=[-1]*len(entities)

		self.cluster_only_nouns(entities, refs, toks, logging=True)
	...
	def cluster_narrator(self, entities, in_quotes, tokens):
		...
		#narrator_pronouns=set(["je", "moi"])
		narrator_pronouns=set(["je", "moi", "ma", "mon", "mes", "moi-même"])
		refs=[]
	...
	def cluster_only_nouns(self, entities, refs, tokens, logging=False):

		# hon_mapper={"mister":"mr.", "mr.":"mr.", "mr":"mr.", "mistah":"mr.", "mastah":"mr.", "master":"mr.",
		# "miss":"miss", "ms.": "miss", "ms":"miss","missus":"miss","mistress":"miss",
		# "mrs.":"mrs.", "mrs":"mrs."
		# }
		#will this work? will it remap all "m's"? should ask this
		hon_mapper={"monsieur":"m.", "m.":"m.", "m":"m.", "madame":"mme", "mme": "mme", 
		...
		return self.cluster(entity_names, is_named, refs, logging=logging)

	def cluster(self, entities, is_named, refs, logging=False):
...
if __name__ == "__main__":
	...
	resolver.process(sys.argv[1], sys.argv[2])
