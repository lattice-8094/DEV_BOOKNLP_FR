		...
		for idx, (start_token, end_token, cat, phrase) in enumerate(entities):
			...
			if tok is not None:
				...
				if tok is not None:
					...
					if tok.deprel == "nsubj" and head.lemma == "être":
						for sibling in toks_by_children[head.token_id]:
							sibling_id = sibling.token_id
							sibling_tok = tokens[sibling_id]
							if (sibling_tok.deprel == "nmod" or sibling_tok.deprel == "amod") and (sibling_tok.pos == "NOUN" or sibling_tok.pos == "ADJ"):
								# Handling modifiers (adjectives) describing the subject
								mods[coref].append({"w": sibling_tok.text, "i": sibling_tok.token_id})
								# Consider further nested relations (e.g., conj for additional modifiers)
						...
						# Functionality for agents (actions that a character does)
					...
					elif tok.deprel == "pobj" and head.deprel == "obl:agent":
						if head.dephead != head.token_id:  # not root
							grandparent = tokens[head.dephead]
							if grandparent.pos_.startswith("V"):
								# Condition for patients (actions done to that character)
								patients[coref].append({"w": grandparent.text, "i": grandparent.token_id})

					elif (tok.deprel == "obj" or tok.deprel == "nsubj:pass") and head.pos == "VERB":
						# Condition for patients (actions done to that character)
						patients[coref].append({"w": head.text, "i": head.token_id})

					elif tok.deprel == "nmod:poss":
